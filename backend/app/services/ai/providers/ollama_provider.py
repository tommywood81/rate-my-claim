...
class OllamaProvider(BaseAIProvider):
    ...
    def __init__(self, base_url: str | None = None) -> None:
        ...
        self._base = (base_url or settings.ollama_base_url or "http://127.0.0.1:11434").rstrip("/")
    ...
    async def _chat(self, prompt: str, model: str = "llama3.2") -> str:
    ...
    async def summarize_evidence(self, context: str) -> str:
        """Short summary."""
        return await self._chat(f"Summarize in <=120 words:\n{context[:12000]}")
    ...
    async def detect_duplicates_llm(self, claim: str, candidates: list[str]) -> list[int]:
        """Duplicate indices."""
        out = await self._chat(f"JSON duplicate_indices for claim vs candidates: {claim} | {candidates}")
        try:
            data = json.loads(out)
    ...
    async def analyze_contradictions(self, claim: str, evidence_blocks: str) -> str:
        ...
        return await self._chat(f"Contradictions? Claim:{claim}\n{evidence_blocks[:8000]}")
