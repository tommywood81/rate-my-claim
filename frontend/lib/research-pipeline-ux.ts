...
export const PIPELINE_STAGES = [
  ...
  { key: "assessed", label: "Check complete", hint: "Verdict, scores, and sources are on your claim page." },
] as const;
  ...
  switch (processingStatus) {
    ...
    case "completed":
      return {
        ...
        researchLabel: "Done",
        decisionLabel: "Done",
        detailMessage: "Check complete. Scores, summary, and matched sources are on your claim page.",
      };
    case "failed":
      return {
        overallPercent: 0,
...
export function submitActiveStageMessage(
  ...
  switch (processingStatus) {
    ...
    case "awaiting_moderation":
    case "completed":
      return "Check complete — open your claim page for the verdict and sources.";
    case "failed":
...
export function buildSubmitOutcomes(
  ...
  if (detail?.public_slug) {
    out.push({ id: "live", text: "Live claim page — visible on Browse." });
  }
  ...
  if (detail?.truth_label && detail.truth_label !== "unclear") {
    out.push({
      id: "truth",
      text: `Verdict: ${formatTruthLabel(detail.truth_label)} (from matched sources + AI read).`,
    });
  } else if (
    detail?.processing_status === "awaiting_moderation" ||
    detail?.processing_status === "completed"
  ) {
    out.push({ id: "truth-unclear", text: "Verdict: inconclusive — not enough sources yet." });
  }
